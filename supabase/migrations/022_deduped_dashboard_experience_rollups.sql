...
create unique index dashboard_experience_unique_records_key_idx
on public.dashboard_experience_unique_records(unique_key);

create index dashboard_experience_unique_records_dataset_idx
on public.dashboard_experience_unique_records(dataset_id);

create index dashboard_experience_unique_records_work_date_idx
on public.dashboard_experience_unique_records(work_date);

create index dashboard_experience_unique_records_employee_idx
on public.dashboard_experience_unique_records(employee);

create index dashboard_experience_unique_records_jobcode_idx
on public.dashboard_experience_unique_records(jobcode);

create index dashboard_experience_unique_records_service_idx
on public.dashboard_experience_unique_records(service_item);

create index dashboard_experience_unique_records_search_trgm_idx
on public.dashboard_experience_unique_records using gin (search_text gin_trgm_ops);
...
begin
  ...
  select jsonb_build_object(
    ...
    'raw_records', raw_scoped.raw_records,
    'unique_records', totals.filtered_timesheets,
    'duplicates_removed', greatest(raw_scoped.raw_records - totals.filtered_timesheets, 0),
    'dataset_count', totals.dataset_count,
    'date_start', totals.date_start,
    'date_end', totals.date_end
  ) into result
  from totals, raw_scoped;
