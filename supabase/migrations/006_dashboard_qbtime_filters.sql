...
begin
  ...
  service_items as (
    select distinct r.json_data #>> '{customfields,53105}' as name
    from public.records r
    join public.datasets d on d.id = r.dataset_id
    where d.name = 'QuickBooks Time Timesheets'
      and coalesce(r.json_data #>> '{customfields,53105}', '') <> ''
  )
  select jsonb_build_object(
    'employees', (select coalesce(jsonb_agg(jsonb_build_object('id', id, 'name', name) order by name), '[]'::jsonb) from employees),
...
begin
  ...
  pto_by_employee as (
    ...
    from (
      ...
      where coalesce(employee_filter, '') = '' or e.employee_id = employee_filter or e.employee_name ilike '%' || employee_filter || '%'
      group by 1
      order by 2 desc
      limit 12
    ) s
...
revoke all on function public.dashboard_qbtime_rollups(text, date, date, text, text, text, text) from public;
grant execute on function public.dashboard_qbtime_rollups(text, date, date, text, text, text, text) to authenticated;
