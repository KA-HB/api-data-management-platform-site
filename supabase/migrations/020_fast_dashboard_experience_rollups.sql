...
begin
  ...
  filtered as materialized (
    select *
    from deduped t
    where (start_date is null or t.work_date >= start_date)
      and (end_date is null or t.work_date <= end_date)
      and (coalesce(employee_filter, '') = '' or t.json_data->>'user_id' = employee_filter or t.employee ilike '%' || employee_filter || '%')
      and (coalesce(jobcode_level1_filter, '') = '' or t.jobcode_level1 ilike '%' || jobcode_level1_filter || '%' or t.jobcode ilike '%' || jobcode_level1_filter || '%')
      and (coalesce(jobcode_level2_filter, '') = '' or t.jobcode_level2 ilike '%' || jobcode_level2_filter || '%' or t.jobcode ilike '%' || jobcode_level2_filter || '%')
      and (coalesce(jobcode_level3_filter, '') = '' or t.jobcode_level3 ilike '%' || jobcode_level3_filter || '%' or t.jobcode ilike '%' || jobcode_level3_filter || '%')
      and (coalesce(service_item_filter, '') = '' or t.service_item ilike '%' || service_item_filter || '%')
      and (
        keyword = ''
        or t.search_text ilike '%' || keyword || '%'
        or lower(t.dataset_name) ilike '%' || keyword || '%'
        or lower(t.employee) ilike '%' || keyword || '%'
        or lower(t.jobcode) ilike '%' || keyword || '%'
        or lower(t.service_item) ilike '%' || keyword || '%'
      )
  ),
  ...
  select jsonb_build_object(
    ...
    'filtered_employees', (select count(distinct nullif(employee, 'Unassigned')) from filtered),
    'filtered_jobcodes', (select count(distinct nullif(jobcode, 'Unassigned')) from filtered),
    'filtered_service_items', (select count(distinct nullif(service_item, 'No service item')) from filtered),
    'raw_records', (select count(*) from base),
