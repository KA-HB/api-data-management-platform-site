-- Allow authenticated users to run anomaly reports within their assigned dataset scope.
...
revoke all on function public.matches_employee_name(text, text, text) from anon;
...
begin
  ...
  classified as (
    select e.*,
      ...
      case
        when excluded_reason is not null or (
          job_history_entries is not null and history_entries is not null
          and not ((combo_billable_rate >= 0.95 and not is_billable) or (combo_billable_rate <= 0.05 and is_billable))
          and history_entries >= minimum_history
        ) then 'ok'
      ...
      case
        ...
        when combo_billable_rate >= 0.95 and not is_billable then format('Historically %s%% billable, but the current entry is marked No', round(combo_billable_rate * 100, 1))
        when combo_billable_rate <= 0.05 and is_billable then format('Historically %s%% billable, but the current entry is marked Yes', round(combo_billable_rate * 100, 1))
...
revoke all on function public.qbtime_anomaly_detection(text, text, date, date, text, text, text, text, uuid, numeric, integer, integer, numeric, integer) from anon;
