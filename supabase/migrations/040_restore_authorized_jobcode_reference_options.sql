-- Restore job-code dropdowns for every authenticated dashboard user by joining
-- authorized time rows to the QuickBooks Time Job Codes reference dataset.
-- The prior regular-user function only read dashboard_experience_unique_records;
-- when those rows held numeric jobcode IDs, clean_jobcode_label removed every
-- option and the dashboard dropdowns went blank.
...
begin
  ...
  used_paths as (
    ...
    join used_jobcodes u on u.raw_id in (p.raw_id, p.parent_raw_id, p.grandparent_raw_id)
  ),
  level1 as (
    select distinct coalesce(grandparent_name, parent_name, name) as id, coalesce(grandparent_name, parent_name, name) as name
    from used_paths
    where public.clean_jobcode_label(coalesce(grandparent_name, parent_name, name)) is not null
    union
  ...
  level2 as (
    select distinct
      case when grandparent_name is not null then parent_name when parent_name is not null then name end as id,
      case when grandparent_name is not null then parent_name when parent_name is not null then name end as name,
      coalesce(grandparent_name, parent_name) as parent_id,
      coalesce(grandparent_name, parent_name) as parent_name
    from used_paths
    where public.clean_jobcode_label(case when grandparent_name is not null then parent_name when parent_name is not null then name end) is not null
      and public.clean_jobcode_label(coalesce(grandparent_name, parent_name)) is not null
    union
  ...
  level3 as (
    ...
    from used_paths
    where grandparent_name is not null
