...
begin
  ...
  enriched as materialized (
    select
      rc.record_id,
      rc.record_dataset_id,
      rc.record_dataset_name,
      rc.record_created_at,
      rc.unique_key,
      rc.record_date,
      rc.duration_seconds,
      rc.employee_name,
      ...
      rc.clean_json_data ||
        jsonb_strip_nulls(jsonb_build_object(
          ...
          'jobcode_name', rc.jobcode_name,
          'jobcode_level1', rc.level1_name,
          'jobcode_level2', rc.level2_name,
          'jobcode_level3', rc.level3_name,
          'service_item', rc.service_item,
          'hours', case when rc.duration_seconds is null then null else round((rc.duration_seconds / 3600)::numeric, 2) end,
          'work_date', rc.record_date
        )) as record_json_data
  ...
  filtered_raw as materialized (
    select
      e.record_id,
      e.record_dataset_id,
      e.record_dataset_name,
      e.record_json_data,
      e.record_created_at,
      e.unique_key
    from enriched e
    where (coalesce(search_term, '') = '' or e.record_json_data::text ilike '%' || search_term || '%' or e.record_dataset_name ilike '%' || search_term || '%')
      and (coalesce(exact_key, '') = '' or coalesce(e.record_json_data->>exact_key, '') = coalesce(exact_value, ''))
      and (start_date is null or e.record_created_at >= start_date or e.record_date >= start_date::date)
      and (end_date is null or e.record_created_at <= end_date or e.record_date <= end_date::date)
      and (coalesce(user_filter, '') = '' or e.record_json_data->>'user_id' = user_filter or e.record_json_data->>'created_by_user_id' = user_filter or e.record_json_data->>'username' ilike '%' || user_filter || '%' or e.record_json_data->>'email' ilike '%' || user_filter || '%')
      and (coalesce(employee_filter, '') = '' or e.record_json_data->>'user_id' = employee_filter or e.employee_name ilike '%' || employee_filter || '%' or concat_ws(' ', e.record_json_data->>'first_name', e.record_json_data->>'last_name') ilike '%' || employee_filter || '%' or e.record_json_data->>'employee_number' = employee_filter)
      and (coalesce(jobcode_filter, '') = '' or e.record_json_data->>'jobcode_id' = jobcode_filter or e.record_json_data->>'name' ilike '%' || jobcode_filter || '%' or e.record_json_data->>'short_code' ilike '%' || jobcode_filter || '%' or e.jobcode_name ilike '%' || jobcode_filter || '%' or e.level1_name ilike '%' || jobcode_filter || '%' or e.level2_name ilike '%' || jobcode_filter || '%' or e.level3_name ilike '%' || jobcode_filter || '%')
      and (coalesce(service_item_filter, '') = '' or e.service_item ilike '%' || service_item_filter || '%' or e.record_json_data->>'service_item' ilike '%' || service_item_filter || '%')
      and (coalesce(status_filter, '') = '' or lower(coalesce(e.record_json_data->>'status', e.record_json_data->>'state', e.record_json_data->>'active', '')) = lower(status_filter))
      and (coalesce(customer_filter, '') = '' or e.record_json_data->>'client_id' = customer_filter or e.record_json_data->>'project_id' = customer_filter or e.record_json_data->>'client' ilike '%' || customer_filter || '%' or e.record_json_data->>'company_name' ilike '%' || customer_filter || '%' or e.record_json_data->>'customer' ilike '%' || customer_filter || '%' or e.record_json_data->>'name' ilike '%' || customer_filter || '%' or e.jobcode_name ilike '%' || customer_filter || '%' or e.level1_name ilike '%' || customer_filter || '%' or e.level2_name ilike '%' || customer_filter || '%' or e.level3_name ilike '%' || customer_filter || '%')
  ),
...
begin
  ...
  record_context as materialized (
    select
      r.id as record_id,
      r.dataset_id as record_dataset_id,
      d.name as record_dataset_name,
  ...
  filtered_raw as materialized (
    select rc.*
    from record_context rc
    where (coalesce(search_term, '') = '' or rc.clean_json_data::text ilike '%' || search_term || '%' or rc.employee_name ilike '%' || search_term || '%' or rc.jobcode_name ilike '%' || search_term || '%' or rc.service_item ilike '%' || search_term || '%' or rc.record_dataset_name ilike '%' || search_term || '%')
      and (coalesce(exact_key, '') = '' or coalesce(rc.clean_json_data->>exact_key, '') = coalesce(exact_value, ''))
      and (start_date is null or rc.record_created_at >= start_date or rc.record_date >= start_date::date)
      and (end_date is null or rc.record_created_at <= end_date or rc.record_date <= end_date::date)
      and (coalesce(user_filter, '') = '' or rc.clean_json_data->>'user_id' = user_filter or rc.clean_json_data->>'created_by_user_id' = user_filter or rc.clean_json_data->>'username' ilike '%' || user_filter || '%' or rc.clean_json_data->>'email' ilike '%' || user_filter || '%')
      and (coalesce(employee_filter, '') = '' or rc.clean_json_data->>'user_id' = employee_filter or rc.employee_name ilike '%' || employee_filter || '%' or concat_ws(' ', rc.clean_json_data->>'first_name', rc.clean_json_data->>'last_name') ilike '%' || employee_filter || '%' or rc.clean_json_data->>'employee_number' = employee_filter)
      and (coalesce(jobcode_filter, '') = '' or rc.clean_json_data->>'jobcode_id' = jobcode_filter or rc.clean_json_data->>'name' ilike '%' || jobcode_filter || '%' or rc.clean_json_data->>'short_code' ilike '%' || jobcode_filter || '%' or rc.jobcode_name ilike '%' || jobcode_filter || '%' or rc.level1_name ilike '%' || jobcode_filter || '%' or rc.level2_name ilike '%' || jobcode_filter || '%' or rc.level3_name ilike '%' || jobcode_filter || '%')
      and (coalesce(service_item_filter, '') = '' or rc.service_item ilike '%' || service_item_filter || '%' or rc.clean_json_data->>'service_item' ilike '%' || service_item_filter || '%')
      and (coalesce(status_filter, '') = '' or lower(coalesce(rc.clean_json_data->>'status', rc.clean_json_data->>'state', rc.clean_json_data->>'active', '')) = lower(status_filter))
      and (coalesce(customer_filter, '') = '' or rc.clean_json_data->>'client_id' = customer_filter or rc.clean_json_data->>'project_id' = customer_filter or rc.clean_json_data->>'client' ilike '%' || customer_filter || '%' or rc.clean_json_data->>'company_name' ilike '%' || customer_filter || '%' or rc.clean_json_data->>'customer' ilike '%' || customer_filter || '%' or rc.clean_json_data->>'name' ilike '%' || customer_filter || '%' or rc.jobcode_name ilike '%' || customer_filter || '%' or rc.level1_name ilike '%' || customer_filter || '%' or rc.level2_name ilike '%' || customer_filter || '%' or rc.level3_name ilike '%' || customer_filter || '%')
  ),
  deduped as materialized (
    select distinct on (fr.record_dataset_id, fr.unique_key) fr.*
    from filtered_raw fr
