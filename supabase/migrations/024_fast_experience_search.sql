...
begin
  ...
  filtered as materialized (
    ...
    where pto_only is not true
      and (start_date is null or e.work_date >= start_date::date)
      and (end_date is null or e.work_date <= end_date::date)
      and (
        term = ''
        or e.search_text ilike '%' || term || '%'
        or lower(e.dataset_name) ilike '%' || term || '%'
        or lower(e.employee) ilike '%' || term || '%'
        or lower(e.jobcode) ilike '%' || term || '%'
        or lower(e.service_item) ilike '%' || term || '%'
      )
      and (
        coalesce(exact_key, '') = ''
        or coalesce(e.json_data->>exact_key, '') = coalesce(exact_value, '')
        or coalesce((jsonb_build_object(
          'employee_name', e.employee,
          'user_id', e.employee_id,
          'jobcode_name', e.jobcode,
          'jobcode_1', e.jobcode_level1,
          ...
          'jobcode_3', e.jobcode_level3,
          'service_item', e.service_item,
          'hours', round(e.hours::numeric, 2)::text,
          'work_date', e.work_date::text
        )->>exact_key), '') = coalesce(exact_value, '')
      )
      and (
        coalesce(user_filter, '') = ''
        or e.employee_id = user_filter
        or e.employee ilike '%' || user_filter || '%'
        or e.json_data->>'username' ilike '%' || user_filter || '%'
        or e.json_data->>'email' ilike '%' || user_filter || '%'
      )
      and (
        coalesce(employee_filter, '') = ''
        or e.employee_id = employee_filter
        or e.employee ilike '%' || employee_filter || '%'
      )
      and (
        coalesce(jobcode_filter, '') = ''
        or e.jobcode ilike '%' || jobcode_filter || '%'
        or e.jobcode_level1 ilike '%' || jobcode_filter || '%'
        or e.jobcode_level2 ilike '%' || jobcode_filter || '%'
        or e.jobcode_level3 ilike '%' || jobcode_filter || '%'
        or e.json_data->>'jobcode_id' = jobcode_filter
      )
      and (
        coalesce(service_item_filter, '') = ''
        or e.service_item ilike '%' || service_item_filter || '%'
      )
      and (
        coalesce(status_filter, '') = ''
        or lower(coalesce(e.json_data->>'status', e.json_data->>'state', e.json_data->>'active', '')) = lower(status_filter)
      )
      and (
        coalesce(customer_filter, '') = ''
        or e.search_text ilike '%' || customer_filter || '%'
        or e.jobcode ilike '%' || customer_filter || '%'
        or e.service_item ilike '%' || customer_filter || '%'
      )
  ),
  counted as (
