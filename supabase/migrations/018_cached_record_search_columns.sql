...
alter table public.records add column if not exists search_text text;
alter table public.records add column if not exists work_date date;
alter table public.records add column if not exists duration_seconds numeric;

create or replace function public.set_record_search_columns()
returns trigger
language plpgsql
set search_path = public
as $$
begin
  new.search_text := lower(coalesce(new.json_data::text, ''));
  new.work_date := coalesce(
    public.safe_date(new.json_data->>'date'),
    public.safe_date(new.json_data->>'local_date'),
    public.safe_date(left(new.json_data->>'start', 10)),
    new.created_at::date
  );
  new.duration_seconds := coalesce(
    public.safe_numeric(new.json_data->>'duration'),
    public.safe_numeric(new.json_data->>'hours') * 3600
  );
  return new;
end;
$$;

drop trigger if exists records_search_columns_tg on public.records;
create trigger records_search_columns_tg
before insert or update of json_data, created_at on public.records
for each row execute function public.set_record_search_columns();

update public.records
set
  search_text = lower(coalesce(json_data::text, '')),
  work_date = coalesce(
    public.safe_date(json_data->>'date'),
    public.safe_date(json_data->>'local_date'),
    public.safe_date(left(json_data->>'start', 10)),
    created_at::date
  ),
  duration_seconds = coalesce(
    public.safe_numeric(json_data->>'duration'),
    public.safe_numeric(json_data->>'hours') * 3600
  )
where true;

create index if not exists records_search_text_trgm_idx
on public.records using gin (search_text gin_trgm_ops);

create index if not exists records_work_date_cached_idx
on public.records(work_date);

create index if not exists records_duration_seconds_idx
on public.records(duration_seconds);
...
begin
  ...
  filtered_raw as (
    ...
    where (term = '' or rs.search_text ilike '%' || term || '%' or lower(rs.record_dataset_name) ilike '%' || term || '%' or lower(coalesce(rs.employee_name, '')) ilike '%' || term || '%' or lower(coalesce(rs.jobcode_name, '')) ilike '%' || term || '%' or lower(coalesce(rs.service_item, '')) ilike '%' || term || '%')
      and (coalesce(exact_key, '') = '' or coalesce(rs.clean_json_data->>exact_key, '') = coalesce(exact_value, ''))
      and (start_date is null or rs.record_date >= start_date::date)
      and (end_date is null or rs.record_date <= end_date::date)
